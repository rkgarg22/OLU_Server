    ...
    if (empty($user)) {
        ...
        if ($status == 1) {
            ...
            if ($user->roles[0] == "contributor") {
                ...
                $getUserDataBooking = $wpdb->get_results("SELECT * FROM `wtw_booking` WHERE `booking_from` = $userID AND `status` != 6  ORDER BY `booking_date` ASC");
                /* echo "<pre>";
                print_r($getUserDataBooking);
                echo "<pre>";
                die(); */
        ...
        if (empty($getUserDataBooking)) {
            ...
            foreach ($getUserDataBooking as $getUserDataBookingkey => $getUserDataBookingvalue) {
                ...
                $price = getBookingPriceUser($getUserDataBookingvalue->id);
                if($price == "") {
                    $price = 0;
                }
